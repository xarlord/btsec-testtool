...
class BleFuzzEngine
    ...
    constructor(
        ...
        suspend fun executeFuzzing(
            ...
            try {
                val validPkt =
                    config.targetCharacteristic?.value
                ...
                val exploitPayloads =
                    if (config.dataPatterns.any { it.patternType == PatternType.KNOWN_EXPLOIT }) {
                        payloadGenerator.generateKnownExploitPayloads().map { it.data }
                    } else {
                        emptyList()
                    }
                ...
                for ((idx, payload) in all.withIndex()) {
                    ...
                    if (!connected && config.stopOnDisconnect) {
                        addFinding(
                            ...
                            sent,
                            payload,
                            "Device disconnected — possible crash",
                            VulnerabilitySeverity.HIGH,
                            FindingCategory.CRASH,
                            "Device ${config.targetDevice.address} disconnected at packet $sent",
                            onFinding,
                        )
                        break
                    }
                    if (config.durationSeconds != null &&
                        Instant.now().epochSecond - startTime.epochSecond >= config.durationSeconds
                    ) {
                        break
                    }
                    ...
                    when (val r = sendPacket(wrapper, config, payload)) {
                        ...
                        is SendResult.Timeout -> {
                            ...
                            addFinding(
                                ...
                                sent,
                                payload,
                                "No response within ${RESPONSE_TIMEOUT_MS}ms — possible hang/DoS",
                                VulnerabilitySeverity.MEDIUM,
                                FindingCategory.HANG,
                        ...
                        is SendResult.Disconnected -> {
                            ...
                            addFinding(
                                ...
                                sent,
                                payload,
                                "Device disconnected after packet — likely crash",
                                VulnerabilitySeverity.CRITICAL,
                                FindingCategory.CRASH,
                                "Hex: ${payload.take(16).joinToString(" ") { "%02x".format(it) }}",
                                onFinding,
                            )
            ...
            return FuzzResult(
                resultId, config, startTime, Instant.now(), status, sent, received,
                errors.toList(), findings.toList(), null, false,
            )
        }

        private fun addFinding(
            list: MutableList<FuzzFinding>,
            pktNum: Int,
            data: ByteArray,
            desc: String,
        ...
        private fun analyzeResponse(
            ...
            sent: ByteArray,
            pktNum: Int,
            findings: MutableList<FuzzFinding>,
            cb: (FuzzFinding) -> Unit,
        ) {
            ...
            if (response.size > 128) {
                addFinding(
                    ...
                    VulnerabilitySeverity.HIGH,
                    FindingCategory.INFORMATION_LEAK,
                    null,
                    cb,
                )
                return
        ...
        private fun connectToDevice(
            config: FuzzConfig,
            cs: MutableStateFlow<ConnectionState>,
        ): GattWrapper =
            try {
                ...
                android.util.Log.w(
                    TAG,
                    "connectToDevice: failed for ${config.targetDevice.address}, falling back to SimGatt — ${e.message}",
                )
                SimGatt(cs)
            }
        ...
        private sealed class GattWrapper {
            ...
            abstract fun readCharacteristic(
                svc: String?,
                chr: String?,
            ): ByteArray?
        ...
        private class RealGatt(
            ...
            init {
                try {
                    ...
                    android.util.Log.e(TAG, "RealGatt: permission denied connecting to ${device.address}", e)
                    connState.value = ConnectionState.Error("Permission denied")
                }
            }
            ...
            override fun writeCharacteristic(
                ...
                return try {
                    ...
                    SendResult.Error(null, "Permission: ${e.message}")
                }
            }
        ...
        private class SimGatt(private val connState: MutableStateFlow<ConnectionState>) : GattWrapper() {
            ...
            override fun writeCharacteristic(
                ...
                if (value.size > 512) {
                    disconnected = true
                    connState.value = ConnectionState.Disconnected
                    return SendResult.Disconnected(null)
                }
