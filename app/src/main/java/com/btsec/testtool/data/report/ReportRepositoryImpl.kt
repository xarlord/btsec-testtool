    ...
    constructor(
        ...
        override fun getReportsSummary(): Flow<ReportsSummary> {
            return flow {
                ...
                // Reduced 4 O(N) iterations and 2 O(N) nested iterations to a single pass,
                // preventing redundant traversals and intermediate allocations.
                // Impact: Time complexity reduced from O(4N + 2(N*V)) to O(N + N*V).
                var draftReportsCount = 0
                var finalReportsCount = 0
                var criticalVulnsCount = 0
                var highVulnsCount = 0

                allReports.forEach { report ->
                    if (report.status == ReportStatus.DRAFT) draftReportsCount++
                    if (report.status == ReportStatus.FINAL) finalReportsCount++
                ...
                emit(
                    ReportsSummary(
                        ...
                        finalReports = finalReportsCount,
                        recentReports = allReports.take(5),
                        criticalVulnerabilitiesTotal = criticalVulnsCount,
                        highVulnerabilitiesTotal = highVulnsCount,
                        pendingActions = 0,
                    ),
                )
            }
