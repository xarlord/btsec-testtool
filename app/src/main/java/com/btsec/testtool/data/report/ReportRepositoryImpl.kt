    ...
    constructor(
        ...
        override fun getReportStatistics(): Flow<ReportStatistics> {
            return flow {
                ...
                // ⚡ Bolt: Calculate stats in single pass to prevent multiple O(n) iterations
                // and reduce intermediate object allocations from groupBy/map operations
                val reportsByStatus = mutableMapOf<ReportStatus, Int>()
                val reportsByMonth = mutableMapOf<String, Int>()
                var totalVulns = 0
                var minDate: Instant? = null
                var maxDate: Instant? = null

                allReports.forEach { report ->
                    reportsByStatus[report.status] = (reportsByStatus[report.status] ?: 0) + 1

                    val monthKey = "${report.generatedAt.toString().substring(0, 7)}"
                    reportsByMonth[monthKey] = (reportsByMonth[monthKey] ?: 0) + 1

                    totalVulns += report.vulnerabilities.size

                    if (minDate == null || report.generatedAt.isBefore(minDate)) minDate = report.generatedAt
                    if (maxDate == null || report.generatedAt.isAfter(maxDate)) maxDate = report.generatedAt
                }

                val averageVulns = if (allReports.isNotEmpty()) totalVulns.toDouble() / allReports.size else Double.NaN
                ...
                emit(
                    ReportStatistics(
                        ...
                        averageVulnerabilitiesPerReport = averageVulns,
                        mostCommonSeverity = VulnerabilitySeverity.MEDIUM,
                        dateRange =
                            DateRange(
                                start = minDate ?: Instant.now(),
                                end = maxDate ?: Instant.now(),
                            ),
                    ),
                )
            }
        ...
        override fun getReportsSummary(): Flow<ReportsSummary> {
            return flow {
                ...
                // ⚡ Bolt: Calculate summary in single pass instead of multiple counts/sumOfs
                // Prevents O(n*m) where n=reports, m=vulnerabilities
                var draftReports = 0
                var finalReports = 0
                var criticalVulns = 0
                var highVulns = 0
                ...
                allReports.forEach { report ->
                    if (report.status == ReportStatus.DRAFT) draftReports++
                    if (report.status == ReportStatus.FINAL) finalReports++
