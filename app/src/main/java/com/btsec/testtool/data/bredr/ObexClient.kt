...
class ObexClient(
     ...
     * @return The server response, or null on transport error.
     */
    fun get(path: String, appParams: ByteArray? = null): ObexResponse? {
        if (!connected) {
     ...
     * @return The server response, or null on transport error.
     */
    fun setPath(path: String, backup: Boolean = false): ObexResponse? {
        if (!connected) return null
        return try {
    ...
    private fun buildSetPathPacket(path: String, backup: Boolean): ByteArray {
    ...
    private fun writeUnicodeHeader(buf: MutableList<Byte>, headerId: Int, value: String) {
    ...
    private fun writeTLVHeader(buf: MutableList<Byte>, headerId: Int, value: ByteArray) {
        buf.add(headerId.toByte())
    ...
    private fun writeFourByteHeader(buf: MutableList<Byte>, headerId: Int, value: ByteArray) {
        buf.add(headerId.toByte())
        buf.add(0x00)
        buf.add(0x00)
    ...
    private fun readResponse(): ObexResponse? {
        return try {
            ...
            val packetLen = ((header[1].toInt() and 0xFF) shl 8) or
        ...
        while (offset < data.size - 1) {
            ...
            when {
                ...
                headerId in 0x00..0x2F || headerId in 0x40..0x4F ||
                    ...
                    val headerLen = ((data[offset].toInt() and 0xFF) shl 8) or
