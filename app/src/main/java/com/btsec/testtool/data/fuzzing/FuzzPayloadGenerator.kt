...
class FuzzPayloadGenerator
    ...
    constructor() {
        ...
        fun generateKnownExploitPayloads(): List<FuzzDataPattern> =
            listOf(
                ...
                FuzzDataPattern(
                    ...
                    PatternType.KNOWN_EXPLOIT,
                    byteArrayOf(
                        0x08, 0x20, 0x0A, 0x00, 0x01, 0x00, L2CAP_CONN_REQ, 0x01,
                ...
                FuzzDataPattern(
                    ...
                    PatternType.KNOWN_EXPLOIT,
                    byteArrayOf(
                        0x0C, 0x20, 0x20, 0x00, 0x01, 0x00, 0x05, 0x02, 0x1C, 0x00,
                ...
                FuzzDataPattern(
                    ...
                    PatternType.KNOWN_EXPLOIT,
                    byteArrayOf(
                        0x02, 0x00, 0x00, 0xFF.toByte(), 0xFF.toByte(), 0xFF.toByte(),
                        0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41,
                    ),
                ),
        ...
        private fun randomBytes(
            rng: Random,
            count: Int,
        ): List<ByteArray> = (0 until count).map { ByteArray(rng.nextInt(MAX_RANDOM_SIZE) + 1) { rng.nextInt(256).toByte() } }

        private fun sequential(
            rng: Random,
            count: Int,
        ): List<ByteArray> = (0 until count).map { i -> ByteArray(rng.nextInt(64) + 1) { j -> ((i + j) % 256).toByte() } }

        private fun lengthFuzz(
            rng: Random,
            count: Int,
            base: ByteArray?,
        ): List<ByteArray> {
            val hdr =
                if (base != null && base.size >= 3) {
                    base.copyOfRange(0, 3)
                } else {
                    byteArrayOf(ATT_WRITE_REQ, 0x00, 0x01)
                }
        ...
        private fun boundary(count: Int): List<ByteArray> {
            val base =
                listOf(
                    byteArrayOf(0x00), byteArrayOf(0xFF.toByte()), byteArrayOf(),
            ...
            return if (count <= base.size) {
                base.take(count)
            } else {
                base + (base.size until count).map { ByteArray(it % 8) { i -> (if (i % 2 == 0) 0 else 0xFF).toByte() } }
            }
        }
        ...
        private fun formatString(count: Int): List<ByteArray> {
            val t =
                listOf(
                    "%s%n%x%d%p", "%n%n%n%n", "%s%s%s%s%s%s%s%s%s%s",
        ...
        private fun protoState(
            ...
            return (0 until count).map {
                val op =
                    if (rng.nextBoolean()) {
                        (ops[rng.nextInt(ops.size)].toInt() xor rng.nextInt(256)).toByte()
                    } else {
                        ops[rng.nextInt(ops.size)]
                    }
