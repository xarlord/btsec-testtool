    ...
    private fun getSafeFile(outputPath: String): Result<File> {
        ...
        return try {
            ...
            val allowedDirs = listOfNotNull(
                context.filesDir,
                context.cacheDir,
                System.getProperty("java.io.tmpdir")?.let { File(it) },
                File("/tmp")
            ).map { it.canonicalPath }
            val isSafe = allowedDirs.any { base ->
                canonicalPath.startsWith(base + File.separator) || canonicalPath == base
            }
            if (isSafe) Result.success(file) else Result.failure(SecurityException("Invalid output path: Path traversal detected or path outside allowed directories"))
        } catch (e: Exception) {
