 ...
 */
@Singleton
class RfcommFuzzingRepositoryImpl @Inject constructor(
    ...
    override suspend fun discoverChannels(deviceAddress: String): List<RfcommChannel> {
        ...
        for (parcelUuid in uuids) {
            ...
            channels.add(
                RfcommChannel(
                    channelNumber = 1, // Actual channel discovered via SDP
    ...
    @SuppressLint("MissingPermission")
    override suspend fun connect(deviceAddress: String, channelNumber: Int): Result<Unit> {
        return try {
    ...
    override suspend fun send(data: ByteArray): Result<ByteArray?> {
        ...
        return try {
            ...
            val read = withTimeoutOrNull(3000L) {
                is2.read(buffer)
            }
