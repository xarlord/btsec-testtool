    ...
    constructor(
        ...
        override fun getFuzzingStatistics(): Flow<FuzzingStatistics> {
            return flow {
                ...
                // Performance optimization: Single-pass iteration to calculate statistics
                // Reduces time complexity from O(n * k) to O(n) and avoids intermediate allocations
                results.forEach { result ->
                    ...
                    val endTime = result.endTime ?: startTime
        ...
        override suspend fun getStatisticsForDevice(deviceAddress: String): DeviceFuzzingStatistics {
            ...
            // Performance optimization: Single-pass iteration to calculate device statistics
            // Reduces time complexity from O(n * k) to O(n) and avoids intermediate allocations
            fuzzingResults.value.forEach { result ->
                if (result.config.targetDevice.address == deviceAddress) {
                    ...
                    val startTime = result.startTime
                    if (lastTestDate == null || startTime.isAfter(lastTestDate)) {
                        lastTestDate = startTime
                    }
                }
            }
