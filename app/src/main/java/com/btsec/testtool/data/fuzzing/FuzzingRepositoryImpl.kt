    ...
    constructor(
        ...
        override fun getFuzzingStatistics(): Flow<FuzzingStatistics> {
            return flow {
                ...
                // ⚡ Bolt: Single-pass iteration to calculate statistics
                // Replaces 8 O(N) operations (4 sumOf, 1 map, 1 average, 1 minByOrNull, 1 maxByOrNull)
                // with a single O(N) pass, avoiding intermediate object allocations
                var totalSent = 0L
                var totalReceived = 0L
                var totalErrorsCount = 0L
                var totalFindingsCount = 0L
                ...
                results.forEach { result ->
                    ...
                    totalErrorsCount += result.errors.size
                    totalFindingsCount += result.findings.size
                ...
                val avgSuccessRate = if (results.isNotEmpty()) totalSuccessRate / results.size else 0.0
        ...
        override suspend fun getStatisticsForDevice(deviceAddress: String): DeviceFuzzingStatistics {
            ...
            // ⚡ Bolt: Single-pass iteration to calculate device statistics
            // Replaces 4 O(N) operations with a single O(N) pass
            var totalSent = 0L
            var totalReceived = 0L
            var totalFindingsCount = 0
            var lastTest: Instant? = null
            ...
            deviceResults.forEach { result ->
                ...
                if (lastTest == null || result.startTime.isAfter(lastTest)) {
                    lastTest = result.startTime
                }
            }
            ...
            return DeviceFuzzingStatistics(
                ...
                lastTestDate = lastTest ?: Instant.now(),
