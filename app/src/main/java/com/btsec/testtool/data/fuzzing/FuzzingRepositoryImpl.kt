    ...
    constructor(
        ...
        override fun getFuzzingStatistics(): Flow<FuzzingStatistics> {
            return flow {
                ...
                // ⚡ Bolt Optimization: Replacing multiple O(n) sumOf passes with a single O(n) loop
                // Expected Impact: Reduces O(6n) traversals to O(n) and prevents intermediate object allocations.
                var packetsSent = 0L
                ...
                emit(
                    FuzzingStatistics(
                        ...
                        mediumFindings = 0,
                        lowFindings = 0,
                        averageSuccessRate = if (results.isNotEmpty()) successRateSum / results.size else 0.0,
        ...
        override suspend fun getStatisticsForDevice(deviceAddress: String): DeviceFuzzingStatistics {
            ...
            // ⚡ Bolt Optimization: Replacing multiple O(n) sumOf passes with a single O(n) loop
            // Expected Impact: Reduces O(4n) traversals to O(n) and prevents intermediate object allocations.
            var packetsSent = 0L
            var packetsReceived = 0L
            var findings = 0
            ...
            deviceResults.forEach { result ->
                ...
                val start = result.startTime
                if (lastTestDate == null || start.isAfter(lastTestDate)) {
                    lastTestDate = start
                }
            }
