    ...
    constructor(
        ...
        override fun getFuzzingStatistics(): Flow<FuzzingStatistics> {
            return flow {
                ...
                // ⚡ Bolt: Use single-pass iteration instead of redundant terminal operations
                ...
                results.forEach {
                    totalPacketsSent += it.packetsSent
                    totalPacketsReceived += it.packetsReceived
                    totalErrors += it.errors.size
                    totalFindings += it.findings.size
                    totalSuccessRate += it.getSuccessRate()

                    if (minStartTime == null || it.startTime < minStartTime!!) {
                        minStartTime = it.startTime
                    }

                    val endTime = it.endTime ?: it.startTime
        ...
        override suspend fun getStatisticsForDevice(deviceAddress: String): DeviceFuzzingStatistics {
            ...
            deviceResults.forEach {
                ...
                findings += it.findings.size
                if (maxStartTime == null || it.startTime > maxStartTime!!) {
                    maxStartTime = it.startTime
                }
            }
