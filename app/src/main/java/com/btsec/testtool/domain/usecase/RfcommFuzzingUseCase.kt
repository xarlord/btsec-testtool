 ...
 */
@Singleton
class RfcommFuzzingUseCase @Inject constructor(
    private val atCommandDictionary: AtCommandDictionary
) {

    companion object {
        private val FORMAT_STRING_PATTERNS = listOf(
            "%s%s%s%s",
            "%x.%x.%x",
            "%n",
            "%0.10000d"
     ...
     */
    fun generatePayload(method: RfcommFuzzMethod, iteration: Int): ByteArray {
        return when (method) {
    ...
    fun computeFuzzStatistics(result: RfcommFuzzResult): String {
        val responseRate = if (result.totalSent > 0) {
            (result.responses.size.toDouble() / result.totalSent * 100).format(1)
        } else {
            "0.0"
        }
        val errorRate = if (result.totalSent > 0) {
