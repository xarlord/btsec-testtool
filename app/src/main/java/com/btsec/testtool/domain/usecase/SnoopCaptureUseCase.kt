...
@Singleton
class SnoopCaptureUseCase @Inject constructor() {
    ...
    fun parseBtsnoopRecord(data: ByteArray, offset: Int): Pair<SnoopRecord, Int> {
        ...
        val packetType = if (recordData.isNotEmpty()) {
    ...
    fun decodeAclPacket(record: SnoopRecord): SnoopL2CapPacket? {
        ...
        val l2capBuf = ByteBuffer.wrap(record.data, l2capOffset, record.data.size - l2capOffset)
    ...
    fun decodePacket(record: SnoopRecord): DecodedPacket {
        ...
        val protocolHint = if (l2cap != null) {
