...
class HfpSecurityUseCase
    ...
    constructor() {
        ...
        private fun detectVulnerability(
            testCase: HfpTestCase,
            response: String?,
        ): Boolean {
        ...
        private fun computeConfidence(
            ...
            when (testCase.category) {
                HfpTestCategory.FORMAT_STRING -> {
                    if (Regex("0x[0-9A-F]+").containsMatchIn(trimmed)) {
                        confidence = 0.95
                ...
                HfpTestCategory.SMS_ACCESS -> {
                    if (trimmed.contains("+CMGL") || trimmed.contains("+CMGR")) {
                        confidence = 0.95
                    } else if (trimmed.contains("OK", ignoreCase = true)) {
                        confidence = 0.7
                    }
                }
        ...
        companion object {
            private val predefinedTestCases: List<HfpTestCase> =
                listOf(
                    // ── Call Manipulation ──
                    HfpTestCase(
