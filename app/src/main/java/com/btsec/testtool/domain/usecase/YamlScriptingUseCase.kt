 ...
 */
@Singleton
class YamlScriptingUseCase @Inject constructor() {
    ...
    companion object {
        ...
        private val ACTIONS_REQUIRING_CONNECT = setOf(
            "read", "write", "fuzz", "subscribe", "service_discovery", "pair", "unpair"
        )
    ...
    fun parseYaml(yamlText: String): YamlParseResult {
        ...
        val variables = (rootMap["variables"] as? Map<*, *>)?.entries?.associate {
     ...
     */
    fun resolveVariables(text: String, variables: Map<String, String>): String {
        ...
        while (i < end) {
            ...
            if (colonIdx != null) {
                ...
                if (rawValue.isEmpty() || rawValue.startsWith("#")) {
                    ...
                    val isList = if (childStart < childEnd) {
                        val firstContentLine = lines.subList(childStart, childEnd)
    ...
    private fun parseStep(
        ...
        @Suppress("UNCHECKED_CAST")
        val params = (raw["params"] as? Map<String, Any>)?.mapValues {
            it.value.toString()
        } ?: emptyMap()

        @Suppress("UNCHECKED_CAST")
        val rawSubsteps = raw["substeps"] as? List<Map<String, Any>> ?: emptyList()
        val substeps = rawSubsteps.mapIndexed { subIndex, subRaw ->
