 ...
 */
@Singleton
class AvrcpSecurityUseCase @Inject constructor() {
     ...
     */
    fun getTestSuite(): List<AvrcpTestCase> = listOf(
     ...
     */
    fun analyzeResponse(testCase: AvrcpTestCase, response: String?): AvrcpTestResult {
     ...
     */
    fun generateReport(report: AvrcpTestReport): String = buildString {
    ...
    private fun isSuccessResponse(response: String): Boolean {
        val indicators = listOf(
            "ok", "success", "accepted", "completed", "acknowledged", "done"
        )
        val lower = response.lowercase()
        return indicators.any { lower.contains(it) }
    }
