            ...
            val thermometerChars =
                listOf(
                    GattCharacteristicConfig(
                        ...
                        permissions = PERMISSION_READ,
                        // 36.7°C IEEE-11073
                        initialValue = byteArrayOf(0x00, 0xE7.toByte(), 0x18.toByte(), 0x00, 0x00),
                        descriptors =
                            listOf(
                                GattDescriptorConfig(
                                    uuid = UUID_CCCD,
            ...
            when (event.eventType) {
                ...
                GattServerEventType.CHARACTERISTIC_READ_REQUEST -> {
                    ...
                    event.response?.let { resp ->
                        sb.appendLine(
                            "  Response status: ${if (resp.status == GATT_SUCCESS) {
                                "SUCCESS"
                            } else {
                                "FAILURE (0x${resp.status.toString(
                                    16,
                                )})"
                            }}",
                        )
