 ...
 */
@Singleton
class InfotainmentCveUseCase
    @Inject
    constructor() {
        /**
         * Hardcoded database of known infotainment Bluetooth CVEs.
         */
        val CVE_DATABASE: List<InfotainmentCve> =
            listOf(
                InfotainmentCve(
                    cveId = "CVE-2018-9313",
         ...
         */
        private val detectionRules: Map<InfotainmentUnit, List<String>> =
            mapOf(
        ...
        fun generateReport(report: InfotainmentTestReport): String {
            ...
            for (result in report.results) {
                if (result.tested) {
                    ...
                    sb.appendLine(
                        "[$status] ${result.cve.cveId} - ${result.cve.name} (CVSS: ${result.cve.cvssScore}, Confidence: ${"%.2f".format(
                            result.confidence,
                        )})",
                    )
