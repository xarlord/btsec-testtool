 ...
 */
@Singleton
class L2capSecurityUseCase @Inject constructor() {

    /**
     * Returns the predefined test suite of L2CAP security test cases.
     */
    fun getTestSuite(): List<L2capTestCase> = listOf(
     ...
     */
    fun buildL2capPacket(cid: Int, payload: ByteArray): ByteArray {
    ...
    fun parseL2capResponse(data: ByteArray): L2capPacket? {
        ...
        val signalingCommand = if (channelId == L2capFixedChannel.SIGNALING.cid ||
            channelId == L2capFixedChannel.LE_SIGNALING.cid
        ) {
            if (payload.isNotEmpty()) L2capSignalCommand.fromCode(payload[0].toInt() and 0xFF)
            else null
        } else null
     ...
     */
    fun analyzeResult(testCase: L2capTestCase, response: ByteArray?): L2capTestResult {
        ...
        when {
            response == null -> {
                vulnerable = testCase.severity in setOf(
