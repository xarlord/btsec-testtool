...
class L2capSecurityRepositoryImplTest {
    ...
    @Test
    fun `enumerateFixedChannels returns empty for null device`() = runTest {
    ...
    @Test
    fun `enumerateFixedChannels returns ATT and SMP for BLE`() = runTest {
        ...
        // Should include ATT and SMP (always available for BLE)
    ...
    @Test
    fun `enumerateFixedChannels includes signaling for BR-EDR devices`() = runTest {
        val mockDevice = mockk<android.bluetooth.BluetoothDevice>(relaxed = true)
        ...
        // Should include BR/EDR signaling for classic profiles
        assertTrue(channels.any { it.cid == 0x0001 }, "Should include Signaling (CID 0x0001)")
    }

    @Test
    fun `sendSignalingCommand returns null (stub implementation)`() = runTest {
        val result = repository.sendSignalingCommand(
        ...
        // Current implementation returns null (requires raw HCI access)
        assertEquals(null, result)
    }

    @Test
    fun `queryInformation returns null (stub implementation)`() = runTest {
        val result = repository.queryInformation(
            deviceAddress = "00:11:22:33:44:55",
            infoType = 1
        )

        // Current implementation returns null (requires raw signaling access)
        assertEquals(null, result)
    }

    @Test
    fun `isL2capConnected initially false`() = runTest {
    ...
    fun `saveTestReport persists and retrieves test report`() = runTest {
        val testReport = L2capTestReport(
    ...
    @Test
    fun `enumerateFixedChannels removes duplicate channels`() = runTest {
        val mockDevice = mockk<android.bluetooth.BluetoothDevice>(relaxed = true)
        every { bluetoothAdapter.getRemoteDevice(any()) } returns mockDevice
        every { mockDevice.uuids } returns emptyArray()
