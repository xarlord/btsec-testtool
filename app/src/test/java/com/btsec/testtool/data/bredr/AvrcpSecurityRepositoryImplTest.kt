...
class AvrcpSecurityRepositoryImplTest {
    ...
    @Test
    fun `isAvrcpConnected initially false`() =
        runTest {
    ...
    @Test
    fun `sendMediaCommand returns failure when not connected`() =
        runTest {
    ...
    @Test
    fun `disconnect closes sockets without throwing`() =
        runTest {
    ...
    @Test
    fun `disconnect cleans up control and browse sockets`() =
        runTest {
            repository.disconnect()
            assertFalse(repository.isAvrcpConnected().first())
        }
    ...
    @Test
    fun `saveTestReport handles reports with browse results`() =
        runTest {
            val testReport =
                AvrcpTestReport(
                    targetDevice = "00:11:22:33:44:55",
    ...
    @Test
    fun `saveTestReport stores results with different severity levels`() =
        runTest {
            val testReport =
                AvrcpTestReport(
                    targetDevice = "00:11:22:33:44:55",
    ...
    @Test
    fun `AvrcpMediaItem can be constructed with all fields`() =
        runTest {
            val item =
                AvrcpMediaItem(
                    uid = 1L,
