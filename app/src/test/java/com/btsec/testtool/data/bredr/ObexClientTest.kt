...
class ObexClientTest {
    ...
    private fun createClient(serverResponses: List<ByteArray>): Pair<ObexClient, ByteArrayOutputStream> {
        ...
        val combined = serverResponses.fold(ByteArrayOutputStream()) { acc, resp ->
            acc.write(resp)
            acc
        }
    ...
    fun `OBEX response parsing handles Unauthorized`() {
        val fakeResponse = byteArrayOf(
            ...
            0x00, 0x03, // Packet length: 3 (no headers)
        )
    ...
    fun `OBEX response with body is parsed correctly`() {
        ...
        // CONNECT OK response
        val connectResponse = byteArrayOf(
            0xA0.toByte(), 0x00, 0x03,
        )
    ...
    fun `OBEX response string header is decoded`() {
        ...
        val (client, _) = createClient(listOf(
    ...
    fun `connect with null target UUID succeeds with OK response`() {
        val response = byteArrayOf(
            0xA0.toByte(), 0x00, 0x07,
            0xC3.toByte(), // Header: Length
            0x00, 0x05,
            0x20.toByte(), 0x00.toByte(), // Max packet 8192
        )
    ...
    fun `maxPacketLength is updated from connect response`() {
        val connectResp = byteArrayOf(
            0xA0.toByte(), 0x00, 0x07,
            0xC3.toByte(), // Header: Length
            0x00, 0x05,
            0x40.toByte(), 0x00.toByte(), // Max packet 16384
        )
    ...
    fun `response body accessor prefers body over end-of-body`() {
        ...
        val data = byteArrayOf(
            0x48.toByte(), ((bodyLen shr 8) and 0xFF).toByte(), (bodyLen and 0xFF).toByte(),
            *body,
            0x49.toByte(), ((eobLen shr 8) and 0xFF).toByte(), (eobLen and 0xFF).toByte(),
            *eob,
        )
